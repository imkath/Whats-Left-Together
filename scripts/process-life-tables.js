...
### Archivos generados
${Object.keys(results).map(key => `- \`${key}.json\``).join('\n')}
...
async function main() {
  ...
  if (!fs.existsSync(CONFIG.inputFile)) {
    ...
    console.error('https://population.un.org/wpp/downloads?folder=Standard+Projections&group=Mortality')
  ...
  return new Promise((resolve, reject) => {
    fs.createReadStream(CONFIG.inputFile)
      ...
      .on('end', () => {
        ...
        console.log(`📦 Generated data for ${Object.keys(results).length} country-sex combinations\n`)
